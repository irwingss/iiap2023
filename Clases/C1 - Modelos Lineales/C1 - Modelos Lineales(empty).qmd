...
remotes::install_github("irwingss/masterX", force = TRUE)
...
comp1 <- testDistribution(rfx_2 %>% filter(pfg == "Cholla") %>% pull(COV),
                          distr = "normal")
plot(comp1)
...
comp2 <- testDistribution(rfx_2 %>% filter(pfg == "Other cacti") %>% pull(COV),
                          distr = "normal")
plot(comp2)
...
qplot(data = rfx_2 %>% filter(pfg == "Cholla"),
      x = YEAR, y = COV)

qplot(data = rfx_2 %>% filter(pfg == "Other cacti"),
      x = YEAR, y = COV)
...
range(rfx_2 %>% filter(pfg == "Cholla") %>% pull(COV), na.rm = TRUE)
range(rfx_2$COV, na.rm = TRUE)

# A2: Linealidad de la relación (Gráfico Residuales Est. vs Valores ajustados)

check_model(mod1_lm_ch, check = "linearity") # Aprox.
check_model(mod2_lm_oc, check = "linearity") # Aprox.

# A3: Normalidad de los residuales (Shapiro)
check_normality(mod1_lm_ch) %>% plot() # No normalidad
masterX::NormTests(vector = residuals(mod1_lm_ch)) # Si

check_normality(mod2_lm_oc) %>% plot() # No normalidad
masterX::NormTests(vector = residuals(mod2_lm_oc)) # Si

check_model(mod1_lm_ch, check = "qq")
check_model(mod2_lm_oc, check = "qq")


# A4: Independencia de los residuales
check_autocorrelation(mod1_lm_ch) # Si
check_autocorrelation(mod2_lm_oc) # si

# A5: Homocedasticidad de los residuales

check_heteroscedasticity(mod1_lm_ch) %>% plot() #si
check_heteroscedasticity(mod2_lm_oc) %>% plot() #no

# A6: Ausencia de valores influyentes (Distancia de Cook)
check_outliers(mod1_lm_ch) # si
check_outliers(mod2_lm_oc) # si
```
...
compare_performance(mod1_lm_ch, mod3_cuad_ch,
                    rank = TRUE) # m1

# Para Otros cactus
compare_performance(mod2_lm_oc, mod4_cuad_oc,
                    rank = TRUE) #m2
```
...
```{r}
g1 <- ggplot(data = rfx_2 %>% filter(pfg == "Cholla"),
             aes(y = COV, x = YEAR)) +
  geom_point(color="deepskyblue1", size=3, alpha = 0.8)
...
# Por cada unidad de aumento de la variable YEAR, el valor esperado de 
# Cobertura de otros cactus aumentará en 0.01 unidades

# Por el paso de un año, el valor esperado de 
# Cobertura de otros cactus aumentará en 0.01 unidades
